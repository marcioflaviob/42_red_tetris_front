...
import { BOARD_COLS, BOARD_ROWS, BUFFER_ZONE_ROWS, CLASS, COLLISION, MOVES, SPAWN_CELL_COL } from "../../utils/constants";
import useBoard from "../../hooks/useBoard";
import useScoreManager from "../../hooks/useScoreManager";

const getCellClassName = (index, type) => {
    if (index < 20 && !type) return styles.bufferZoneCell;
    return type === CLASS.YELLOW ? styles.filled : type === CLASS.PREDICT ? styles.predict : styles.cell;
}
...
const GameCard = ({ player, setScore, level, setLevel }) => {
  ...
  const boardCells = useMemo(() => {
    ...
    const cells = useMemo(() => {
        ...
        const predictIndices = new Set(activePiece?.predictCoords?.map(coords => getIndex(coords)));
        
        return boardCells.map(({ idx, filled }) => (
            <Cell 
                key={idx} 
                index={idx} 
                type={activePieceIndices.has(idx) || filled ? CLASS.YELLOW : predictIndices.has(idx) ? CLASS.PREDICT : CLASS.EMPTY} 
            />
        ));
